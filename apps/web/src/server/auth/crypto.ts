...
function decodeStoreKey(value: string): Uint8Array {
  ...
  catch { throw new NetslumError("WORKER_FAILED", "OAuth encryption key is invalid", 500); }
  if (bytes.byteLength !== 32) throw new NetslumError("WORKER_FAILED", "OAuth encryption key must be 32 bytes", 500);
  return bytes;
}
